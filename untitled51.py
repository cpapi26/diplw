...
import pandas as pd  # ΠΡΟΣΘΗΚΗ: Εισαγωγή pandas για CSV
# =====================================================
# ΒΟΗΘΗΤΙΚΕΣ ΣΥΝΑΡΤΗΣΕΙΣ (από τον δεύτερο κώδικα)
# =====================================================
# =====================================================
# ΝΕΕΣ ΣΥΝΑΡΤΗΣΕΙΣ ΓΙΑ ΑΝΑΛΥΤΙΚΗ ΤΟΜΗ
# =====================================================
...
# =====================================================
# ΑΝΑΛΥΤΙΚΗ ΔΙΑΔΡΟΜΗ 3D (από τον πρώτο κώδικα)
# =====================================================
...
class AnalyticalRailPath3D:
    ...
    def __init__(self, 
        ...
        # Προκαθορισμός των σημείων
        self._precompute_path()

    def _precompute_path(self):
        """Προϋπολογισμός του path για στροφή"""
        if self.wheel_type == "left":
            ...
            # 2. Clothoid με banking (20-40m+ σε z, 0->100mm σε y)
            ...
            x2 = -A * C2
            z2 = A * S2
            x2 = x2 - x2[0]  # Μετατόπιση για συνέχεια
            z2 = z2 - z2[0] + self.L_straight  # Μετατόπιση για συνέχεια

            # y2: banking ανεβαίνει 0 -> h στα πρώτα L_incline, μετά σταθερό
            y2 = np.zeros_like(x2)
            incline_mask = s2 <= self.L_incline
            y2[incline_mask] = np.interp(s2[incline_mask], 
                                       [0, self.L_incline], 
                                       [0, self.h])
        ...
        # Δημιουργία spline για αναλυτική προσπέλαση
        self._create_splines()

    def _create_splines(self):
        """Δημιουργία spline συναρτήσεων για αναλυτική προσπέλαση"""
    ...
    def tangent(self, s):
        ...
        # Για σημεία με μηδενική παράγωγο (ασφάλεια)
        dx[~valid_norms] = 0
        dy[~valid_norms] = 0
        dz[~valid_norms] = 1
    ...
    def curvature(self, s):
        """
        Επιστρέφει την καμπυλότητα.
        """
        s = np.array(s, ndmin=1, dtype=float)
        s_clipped = np.clip(s, 0, self.s_total)
        
        # Αριθμητικός υπολογισμός δεύτερης παραγώγου
    ...
    def superelevation(self, s):
        """
        Επιστρέφει την ανύψωση (banking) σε radians.
        """
        s = np.array(s, ndmin=1, dtype=float)
        s_clipped = np.clip(s, 0, self.s_total)
        
        # Για αριστερή ράγα: banking από 0 έως arctan(h/gauge)
        if self.wheel_type == "left":
            max_banking = np.arctan(self.h / self.gauge)
            
            # Banking αυξάνεται γραμμικά στο L_incline, μετά σταθερό
            ...
            # Στην ευθεία: μηδέν banking
            banking[straight_mask] = 0
            
            # Στο clothoid: γραμμική αύξηση
            clothoid_mask = incline_mask & ~straight_mask
            s_clothoid = s_clipped[clothoid_mask] - self.L_straight
            banking[clothoid_mask] = np.interp(s_clothoid, 
                                             [0, self.L_incline], 
                                             [0, max_banking])
            
            # Μετά το L_incline: σταθερό banking
            banking[~incline_mask] = max_banking
            
            return banking
        
        else:  # Δεξιά ράγα: χωρίς banking
            return np.zeros_like(s_clipped)

    def get_path_points(self, num_points=1000):
        """
        Επιστρέφει διακριτά σημεία του path για οπτικοποίηση.
        """
        s_values = np.linspace(0, self.s_total, num_points)
        x, y, z = self.position(s_values)
        return np.column_stack([x, y, z])

# =====================================================
# Παράδειγμα ενσωμάτωσης στο υπάρχον WheelRailSystemAnalytical
# =====================================================
class WheelRailSystemAnalytical:
    def __init__(self, wheel_center, wheel_quat, V, omega, wheel_type="right"):
        ...
        self.create_analytical_path()

    def create_analytical_path(self):
        """Δημιουργία του αναλυτικού path"""
        self.analytical_path = AnalyticalRailPath3D(wheel_type=self.wheel_type)
        
        self.path_func = {
            'length': self.analytical_path.s_total,
            'gauge': self.analytical_path.gauge
        }
        
        # Μετατόπιση ανάλογα με τον τύπο τροχού
        if self.wheel_type == "left":
            self.x_shift = 0
        else:
            self.x_shift = 0
    ...
    def get_path_point(self, s):
        ...
        return np.array([x + self.x_shift, y + self.y_shift, z])
...
def sweep_profile_point_3d(profile_point_2d, s, path_func, twist_func):
    ...
    # Debugging: έλεγχος σχήματος
    print(f"Point shape: {pt_3d.shape}")  # Πρέπει να είναι (3,)
...
def GFOSUB_enhanced_analytical(wheel_center, V, omega, euler_angles, wheel_type="right", return_plot_data=True):
    ...
    def path_func(s):
        ...
        # Βεβαιωθείτε ότι επιστρέφονται 4 τιμές
    ...
    # ΠΡΟΣΘΗΚΗ: Έλεγχος για κενό rail_poly
    if len(rail_poly) == 0:
        print("Warning: No rail polygon found - using default rail profile")
        # Επιστροφή προφίλ ράγας χωρίς μετασχηματισμό
        rail_poly = np.column_stack([system.RAIL_POINTS_X, system.RAIL_POINTS_Y])
        # Κλείσιμο του polygon
        rail_poly = np.vstack([rail_poly, rail_poly[0]])
...
# Κλήση της συνάρτησης μετά το plot

# =====================================================
# ΠΑΡΑΔΕΙΓΜΑ ΧΡΗΣΗΣ
# =====================================================
if __name__ == "__main__":
    wheel_center = [746, 589, 450]
    V = [0, 0, 1000]
    omega = [0, 10, 0]
    ...
    results = GFOSUB_enhanced_analytical(
        ...
        wheel_type="left", return_plot_data=True
    )
    
    # Οπτικοποίηση και αποθήκευση
